...
// =====================================================================
//  __      __      __
//  \ \    /  \    / /  [File name   ] gowin_pcie_drv
//   \ \  / /\ \  / /   [Description ] Source file for PCIe driver
...
// V2.0 | Vladimir Zaikin | 2026/05/12 | Adapt to kernel versions & BAR
// V2.1 | Vladimir Zaikin | 2026/08/14 | Clean up driver & Rename
// =========Oooo===========================================Oooo=========
 ...
 *
 * @param[in]   data:   pointer to struct gowin_pcie_data
 * @param[inout] arg:   parameters for ioctl
 */
static int ioctl_read_config(struct gowin_pcie_data *data, unsigned long arg) {
 ...
 * @param[inout] arg:   parameters for ioctl
 */
static int ioctl_write_config(struct gowin_pcie_data *data, unsigned long arg) {
    struct gowin_ioctl_param param;
    struct device *dev;
 ...
 * @param[inout] arg:   parameters for ioctl
 */
static int ioctl_dma_mem_request(struct gowin_pcie_data *data, unsigned long arg) {
    int id, size;
    struct gowin_ioctl_param param;
    struct device *dev;
 ...
 * @param[inout] arg:   parameters for ioctl
 */
static int ioctl_dma_mem_release(struct gowin_pcie_data *data, unsigned long arg) {
    int id;
    struct gowin_ioctl_param param;
    struct device *dev;
 ...
 * @param[inout] arg:   parameters for ioctl
 */
static int ioctl_switch_bar_mem(struct gowin_pcie_data *data, unsigned long arg) {
    int index;
    struct gowin_ioctl_param param;
    struct device *dev;
 ...
 * @param[inout] arg:   parameters for ioctl
 */
static int ioctl_debug(struct gowin_pcie_data *data, unsigned long arg) {
    int id, size;
    struct gowin_ioctl_param param;
    struct device *dev;
...
static long gowin_pcie_ioctl(struct file *filp, unsigned int cmd,
                             unsigned long arg) {
    struct device *dev;
    struct gowin_pcie_data *data = filp->private_data;
    int err = 0;
...
static int gowin_pcie_open(struct inode *inode, struct file *filp) {
    int err;
    unsigned long flags;
    struct device *dev;
    struct cdev *cdev = inode->i_cdev;
    struct gowin_pcie_data *data =
        container_of(cdev, struct gowin_pcie_data, gw_cdev);
    ...
    if (data->open_cnt) {
        ...
        dev_dbg(dev, "gowin_pcie_open() DEBUG.\n");
        err = 0;
        data->open_cnt++;
    }
 ...
 *
 */
static int gowin_pcie_mmap(struct file *filp, struct vm_area_struct *vma) {
    int ret;
    struct device *dev;
    struct gowin_pcie_data *data = filp->private_data;
    void *vir;
    ...
    if (vsize > psize || psize <= 0) {
        dev_err(dev, "gowin_pcie_mmap() failed.\n");
        return -EINVAL;
    }
...
static int gowin_pcie_release(struct inode *inode, struct file *filp) {
    unsigned long flags;
    struct device *dev;
    struct gowin_pcie_data *data = filp->private_data;
    ...
    dev_dbg(dev, "gowin_pcie_release() DEBUG.\n");
...
static const struct file_operations gowin_pcie_fops = {
    .owner = THIS_MODULE,
    .unlocked_ioctl = gowin_pcie_ioctl,
    .compat_ioctl = compat_ptr_ioctl,
    .llseek = noop_llseek,
    .open = gowin_pcie_open,
    .mmap = gowin_pcie_mmap,
    .release = gowin_pcie_release,
};

static int gowin_pcie_probe(struct pci_dev *pdev, const struct pci_device_id *did) {
    int err;
    int bar;
    struct gowin_pcie_data *data;
    ...
    cdev_init(&data->gw_cdev, &gowin_pcie_fops);
...
static void gowin_pcie_remove(struct pci_dev *pdev) {
    struct gowin_pcie_data *data = pci_get_drvdata(pdev);
...
static struct pci_driver gowin_pcie_driver = {
    .name = DRIVER_NAME,
    .id_table = gowin_tbl,
    .probe = gowin_pcie_probe,
    .remove = gowin_pcie_remove,
};

module_pci_driver(gowin_pcie_driver);

MODULE_DESCRIPTION("PCIe Driver");
MODULE_AUTHOR("Huang Mingtao <mingtao@gowinsemi.com>, "
              "Vladimir Zaikin <friend.zva@yandex.ru>");
MODULE_LICENSE("GPL");
